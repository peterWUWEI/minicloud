                ...
                <material-card color="green" title="服务编辑" text="更改服务内容">
                    <v-form v-model="isFormValid" @submit.prevent="updateService">
                        <v-container class="py-0">
                            <v-row>
                                <v-col cols="12" md="4">
                                    <v-text-field v-model="service_id" label="ID (disabled)" disabled filled />
                                </v-col>
...
export default {
    ...
    async created() {
        try {
            ...
            this.service_title = res.data.service_title;
    ...
    methods: {
        async updateService() {
            const res = await this.$axios.put(`/services/${this.$route.params.id}`, {
